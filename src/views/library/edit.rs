...
pub(crate) fn fields_for(section: SectionKind) -> &'static [FieldSpec] {
    match section {
...
pub(crate) fn block_noun(section: SectionKind) -> &'static str {
    match section {
...
/// The identity of the block at `index`, as [`super::usage`] computes
...
fn library_link_identity(library: &Library, section: SectionKind, index: usize) -> Option<String> {
    super::usage::block_identity(library, section, index)
}
...
impl Shell {
    ...
    pub(crate) fn open_library_edit(
        &mut self,
        section: SectionKind,
        index: Option<usize>,
        window: &mut Window,
    ...
    pub(crate) fn close_library_edit(&mut self, cx: &mut Context<Self>) {
    ...
    fn commit_library_edit(&mut self, cx: &mut Context<Self>) {
        ...
        let radius = index.and_then(|index| {
            ...
            let targets = super::link::targets_for(
                &library,
    ...
    pub(crate) fn render_library_edit_sheet(&self, cx: &mut Context<Self>) -> Option<AnyElement> {
