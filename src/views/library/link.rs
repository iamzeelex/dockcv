...
impl Shell {
    ...
    pub(crate) fn close_push_review(&mut self, cx: &mut Context<Self>) {
    ...
    fn run_push(&mut self, cx: &mut Context<Self>) {
        ...
        for target in review.targets.iter().filter(|t| t.selected) {
            ...
            let rewritten = super::usage::replace_matching(
                &mut doc,
                review.section,
                &review.identity,
                &library,
    ...
    pub(crate) fn render_library_push_sheet(&self, cx: &mut Context<Self>) -> Option<AnyElement> {
