...
use super::front_door::{groups, history_line, is_generated_name, page_count_label, Group, Reading};
...
impl Shell {
    ...
    pub(super) fn render_readings(&self, cx: &mut Context<Self>) -> impl IntoElement {
    ...
    /// off the CV's *name* — see `front_door_menus.rs` — so there is no second
    ...
    pub(super) fn render_add_cv(&self, cx: &mut Context<Self>) -> AnyElement {
    ...
    pub(super) fn render_tailor_callout(&self, cx: &mut Context<Self>) -> impl IntoElement {
        let theme = *cx.theme();
        div()
            .mb(px(24.0))
            .flex()
    ...
    pub(super) fn front_door_title(&self) -> String {
        let mut names = self
            .cache
            .metadata()
            .iter()
