...
//! `front_door_menus.rs` then had to fix the appearance, because two identical
...
pub(super) struct VersionRename {
    pub path: PathBuf,
    pub index: usize,
...
impl Shell {
    ...
    pub(super) fn start_version_rename(
        &mut self,
        path: &Path,
        index: usize,
        window: &mut Window,
    ...
    pub(super) fn commit_version_rename(&mut self, cx: &mut Context<Self>) {
    ...
    pub(super) fn duplicate_version(&mut self, path: &Path, index: usize, cx: &mut Context<Self>) {
    ...
    pub(super) fn delete_version(&mut self, path: &Path, index: usize, cx: &mut Context<Self>) {
        let Ok(mut doc) = vault::load(path) else {
            return;
        };
