...
impl Shell {
    ...
    pub(crate) fn cancel_batch_export(&mut self, cx: &mut Context<Self>) {
    ...
    /// relabelled.
    pub(crate) fn set_batch_collision_policy(
        &mut self,
        policy: OnCollision,
        cx: &mut Context<Self>,
    ) {
        ...
        let plan = plan_batch(
            ...
            &sheet.folder,
            "pdf",
            &crate::views::root_export_sheet::today(),
            policy,
        );
    ...
    pub(crate) fn render_batch_export_sheet(&self, cx: &mut Context<Self>) -> AnyElement {
        let theme = *cx.theme();
        let Some(sheet) = &self.batch_export else {
