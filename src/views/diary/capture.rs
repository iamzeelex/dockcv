...
pub(crate) fn candidates(pasted: &str) -> Vec<Candidate> {
...
impl Shell {
    pub(crate) fn open_diary_paste(&mut self, window: &mut Window, cx: &mut Context<Self>) {
    ...
    pub(crate) fn close_diary_paste(&mut self, cx: &mut Context<Self>) {
    ...
    pub(crate) fn render_diary_paste_sheet(&self, cx: &mut Context<Self>) -> Option<AnyElement> {
