...
//! holds the state, `preset_matrix_grid.rs` draws it, and the actions that
...
impl Shell {
    ...
    pub(super) fn measure_matrix_pages(&mut self, cx: &mut Context<Self>) {
        let Screen::PresetMatrix(ref mut pm) = self.screen else {
            return;
        };
        if pm.measuring {
    ...
    pub(super) fn load_matrix_records(&mut self) {
