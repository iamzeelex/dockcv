...
use super::snapshot::pin_groups;
use crate::views::shell::Shell;

/// The open CV picker.
pub(crate) struct PinPick {
...
impl Shell {
    ...
    pub(crate) fn close_pin_pick(&mut self, cx: &mut Context<Self>) {
        self.pin_pick = None;
        cx.notify();
    }
    ...
    pub(crate) fn render_pin_pick_sheet(&self, cx: &mut Context<Self>) -> Option<AnyElement> {
        ...
        let groups: Vec<_> = pin_groups(self.cache.metadata())
            ...
            .filter_map(|group| {
                ...
                let presets: Vec<String> = group
                    ...
                    .collect();
                (!presets.is_empty())
                    .then_some(super::snapshot::PinGroup { presets, ..group })
            })
            .collect();
