...
pub(crate) fn pin_groups(metas: &[vault::DocMeta]) -> Vec<PinGroup> {
    metas
        .iter()
...
impl Shell {
    ...
    pub(crate) fn pin_application_cv(
        &mut self,
        index: usize,
        stem: String,
        preset: String,
    ...
    pub(crate) fn unpin_application_cv(&mut self, index: usize, cx: &mut Context<Self>) {
        let Some(vault) = self.vault.clone() else {
            return;
        };
        let mut applications = vault::load_applications(&vault);
    ...
    pub(crate) fn capture_snapshot(&mut self, index: usize, cx: &mut Context<Self>) {
        let Some(vault) = self.vault.clone() else {
            return;
        };
    ...
    pub(crate) fn reveal_snapshot(&mut self, index: usize, cx: &mut Context<Self>) {
        let Some(vault) = self.vault.clone() else {
            return;
        };
        let applications = vault::load_applications(&vault);
