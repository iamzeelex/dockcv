...
pub(crate) fn matches_query(app: &Application, query: &str) -> bool {
    query.is_empty()
...
pub(crate) fn card_chip_text(app: &Application, status: ApplicationStatus) -> Option<String> {
    match status {
...
pub(crate) fn next_step_caption(step: &NextStep) -> String {
...
pub(crate) fn interviews_this_week(applications: &Applications, today: &str) -> usize {
    applications
        .entries
        .iter()
...
pub(crate) fn parse_iso(date: &str) -> Option<(i32, u32, u32)> {
...
pub(crate) fn monday_index(days: i64) -> i64 {
    (days - 4).rem_euclid(7)
}
...
pub(crate) fn weekday_abbrev(date: &str) -> Option<&'static str> {
...
pub(crate) fn same_week(date: &str, today: &str) -> bool {
...
pub(crate) fn iso_to_epoch_secs(date: &str) -> Option<u64> {
    let (y, m, d) = parse_iso(date)?;
    let days = days_from_civil(y as i64, m, d);
    (days >= 0).then(|| days as u64 * 86_400)
}

pub(crate) fn relative_from_iso(date: &str, now_secs: u64) -> Option<String> {
...
pub(crate) enum ApplicationsView {
    #[default]
    Board,
    List,
    Insights,
...
impl ApplicationsView {
    pub(crate) const ALL: [ApplicationsView; 3] = [
...
/// one thing.
#[derive(Clone, Copy, Debug, Default, PartialEq, Eq)]
pub(crate) enum ApplicationSort {
...
impl ApplicationSort {
    pub(crate) const ALL: [ApplicationSort; 6] = [
...
pub(crate) fn last_touched(app: &Application) -> &str {
...
pub(crate) fn sort_rows<A: std::borrow::Borrow<Application>>(
    rows: &mut [(usize, A)],
...
pub(crate) const fn status_title(status: ApplicationStatus) -> &'static str {
    match status {
