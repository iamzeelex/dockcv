...
use super::card::column_tint;
use super::data::{short_date, status_title};
...
impl Shell {
    ...
    pub(crate) fn open_application_detail(
        &mut self,
        index: usize,
        window: &mut Window,
        cx: &mut Context<Self>,
    ...
    pub(crate) fn focus_application_company(
        &mut self,
        window: &mut Window,
        cx: &mut Context<Self>,
    ) {
    ...
    pub(crate) fn discard_application_detail(&mut self, cx: &mut Context<Self>) {
        ...
        if let Some(vault) = self.vault.clone() {
            ...
            if applications.entries.get(detail.index).is_some() {
                crate::views::shell::remove_at(&mut applications.entries, detail.index);
                save_status::record(
                    cx,
                    "applications board",
    ...
    pub(crate) fn close_application_detail(&mut self, cx: &mut Context<Self>) {
        self.commit_application_detail(cx);
        let Some(detail) = self.applications_detail.take() else {
            return;
        };
    ...
    pub(crate) fn commit_application_detail(&mut self, cx: &mut Context<Self>) {
    ...
    pub(crate) fn log_interview_round(
        &mut self,
        index: usize,
        window: &mut Window,
        cx: &mut Context<Self>,
    ...
    pub(crate) fn set_application_closure(
        &mut self,
        index: usize,
    ...
    pub(crate) fn render_application_detail(&self, cx: &mut Context<Self>) -> Option<AnyElement> {
...
pub(crate) fn index_after_removal(open: usize, removed: usize) -> Option<usize> {
