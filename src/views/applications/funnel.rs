...
//!
//! Pure — no `gpui`, no `Theme`. The chart in `applications/analytics.rs`
...
impl Outcome {
    ...
    pub(crate) fn of(app: &Application) -> Option<Self> {
...
pub(crate) const NO_PRESET: &str = "No preset recorded";
...
impl Funnel {
    pub(crate) fn of(applications: &Applications) -> Self {
    ...
    pub(crate) fn total(&self, outcome: Outcome) -> usize {
        self.flows
            .iter()
    ...
    pub(crate) fn interview_rate(&self) -> Option<f32> {
        (self.sent > 0).then(|| {
...
pub(crate) fn missing_history(applications: &Applications) -> usize {
    applications
        .entries
        .iter()
...
pub(crate) fn stage_flow(applications: &Applications, from: &str, to: &str) -> Vec<StageFlow> {
...
pub(crate) fn journey(applications: &Applications) -> Journey {
