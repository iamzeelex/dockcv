...
//! says where you are.
//!
//! What goes *in* the left panel is `import/panels.rs` — the drop zone, the
...
pub(crate) mod assistant_view;
pub(crate) mod panels;
pub(crate) mod unplaced;
...
impl Shell {
    ...
    fn render_import_panel(&self, cx: &mut Context<Self>) -> AnyElement {
        use super::import::panels;
        match &self.import_step {
            ImportStep::Step1Drop => panels::render_drop_panel(
                cx,
            ...
            ImportStep::Parsing { filename } => {
                panels::render_parsing_step(cx, filename).into_any_element()
            }
            ImportStep::CouldNotRead {
                filename,
                path,
                ...
                .flex()
                .flex_col()
                .child(panels::render_could_not_read(
                    cx,
                    filename,
                    error,
                    |this, cx| {
                ...
                .into_any_element(),
            ImportStep::Step2Review { imported } => panels::render_step2_review_split(
                cx,
                imported,
