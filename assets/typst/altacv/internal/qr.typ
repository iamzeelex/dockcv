...
// DOCKCV FORK: `@preview/zebra` is not vendored — QR codes are an AltaCV
// feature DockCV does not offer, and a package that cannot be downloaded
// (US-10) cannot be a hard dependency of one that can. `_check_qr_code` is
// left exactly as upstream wrote it, so a document that asks for a QR code
// still gets a clear message instead of a missing element.
...
// DOCKCV FORK: upstream renders `qrcode(url, ..)` from `@preview/zebra` here.
#let _qr_render(url, size, fill) = panic(
  "QR codes are not available in DockCV: the `zebra` package they need cannot "
    + "be downloaded at runtime. Remove `qrCode` from the document's "
    + "preferences.",
)
